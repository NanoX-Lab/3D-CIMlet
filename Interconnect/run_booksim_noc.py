...
def run_booksim_noc(config,trace_file_dir,num_used_static_chiplet_all_layers, num_used_dynamic_chiplet, chiplet_static_type):
    ...
    target_dir = os.path.join(current_dir, "logs", "configs")
    # Create directory to store config files
    os.makedirs(target_dir, exist_ok=True)

    # Open the file and check if it has content
    area_file_path = os.path.join(current_dir, "logs", "Area_chiplet.csv")
    ...
    # Open the file and check if it has content
    latency_file_path = os.path.join(current_dir, "logs", "Latency_chiplet.csv")
    # Check if the file exists and if it is empty
    ...
    # Open the file and check if it has content
    power_file_path = os.path.join(current_dir, "logs", "Energy_chiplet.csv")
    # Check if the file exists and if it is empty
    ...
    for chiplet_idx in range(0, num_chiplets):
        ...
        for file in files:
            ...
            fp = open(os.path.join(current_dir, "mesh_config_trace_based"), 'r')
            
            # Set path to config file
            config_file = os.path.join(current_dir, "logs", "configs", f"chiplet_{chiplet_idx}_mesh_config")

            # Open write file handle for config file
            outfile = open(config_file, 'w')
            
            # Set the new tech file name
            new_tech_file = 'techFiles/' + 'techfile_' + str(chip_technode) + 'nm.txt'
            ...
            log_file = os.path.join(current_dir, "logs", f"chiplet_{chiplet_idx}_layer_{run_id}.log")
        ...
        outfile_area = open(os.path.join(current_dir, "logs", "booksim_area.csv"), 'a')

        if file_counter == 0:
            # print('No NoC for this Chiplet.')
            outfile_area.write(str(0) + '\n')
            outfile_area.close()
            area_file = open(os.path.join(current_dir, "logs", "Area_chiplet.csv"), 'a')
        ...
        outfile_latency = open(os.path.join(current_dir, "logs", "booksim_latency.csv"), 'a')
        outfile_latency.write(str(total_latency) + '\n')
        outfile_latency.close()
        latency_file = open(os.path.join(current_dir, "logs", "Latency_chiplet.csv"), 'a')
        latency_file.write('Total NoC latency is' + '\t' + str(total_latency*1/chip_clk_freq) + '\t' + 's' + '\n')
        latency_file.close()
    
        # Open output file handle to write power
        outfile_power = open(os.path.join(current_dir, "logs", "booksim_power.csv"), 'a')
        
        if file_counter == 0:
            outfile_power.write(str(0) + '\n')
            outfile_power.close()
            power_file = open(os.path.join(current_dir, "logs", "Energy_chiplet.csv"), 'a')
